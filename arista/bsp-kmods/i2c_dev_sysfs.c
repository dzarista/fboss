...
ssize_t i2c_dev_show_label(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
...
int i2c_dev_read_byte(struct device *dev, struct device_attribute *attr)
{
	...
	val_mask =
		~(((-1) >> (dev_attr->ida_n_bits)) << (dev_attr->ida_n_bits));
...
int i2c_dev_read_nbytes(struct device *dev, struct device_attribute *attr,
			uint8_t values[], int nbytes)
{
	...
	for (i = 0; i < nbytes; ++i) {
		ret_val =
			i2c_smbus_read_byte_data(client, dev_attr->ida_reg + i);
		if (ret_val < 0) {
			mutex_unlock(&data->idd_lock);
			return ret_val;
		}
...
ssize_t i2c_dev_show_ascii(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	i2c_sysfs_attr_st *i2c_attr = TO_I2C_SYSFS_ATTR(attr);
	const i2c_dev_attr_st *dev_attr = i2c_attr->isa_i2c_attr;
...
unlock_out:
	...
	if (dev_attr->enable_log_on_write) {
		pr_crit("Wrote %#x to %s -> %s by pid %ld (cmd=%s)\n", req_val,
			client->name, i2c_attr->isa_dev_attr.attr.name,
...
int i2c_dev_sysfs_data_init(struct i2c_client *client, i2c_dev_data_st *data,
			    const i2c_dev_attr_st *dev_attrs, int n_attrs)
{
	int i;
	int err;
	...
	data->idd_attrs =
		kzalloc(sizeof(*data->idd_attrs) * n_attrs, GFP_KERNEL);
	/* allocate the attribute group. +1 for the last NULL */
	data->idd_attr_group.attrs =
		kzalloc(sizeof(*data->idd_attr_group.attrs) * (n_attrs + 1),
			GFP_KERNEL);
	...
	for (i = 0, cur_attr = &data->idd_attrs[0],
	    cur_grp_attr = &data->idd_attr_group.attrs[0],
	    cur_dev_attr = dev_attrs;
	     i < n_attrs; i++, cur_attr++, cur_grp_attr++, cur_dev_attr++) {
		mode_t mode = S_IRUGO;
		...
		*cur_grp_attr = &cur_attr->isa_dev_attr.attr;
		...
		PP_DEBUG("Created attribute \"%s\"",
			 cur_attr->isa_dev_attr.attr.name);
	}
	...
	data->idd_hwmon_dev = hwmon_device_register_with_groups(
		&client->dev, client->name, NULL, NULL);
...
MODULE_AUTHOR("Tian Fang <tfang@meta.com>");
