...
class FanServiceConfig:
   ...
   def ledJson(self):
      led_data = OrderedDict()
      fru_type_configs = OrderedDict()
      fru_configs = OrderedDict()

      led_data[ "systemLedConfig" ] = self.getLedConfig( "sys" )

      for fru_name in [ "FAN", "PSU" ]:
         fru_type_configs[ fru_name ] = self.getLedConfig( fru_name.lower() )
      led_data[ "fruTypeLedConfigs" ] = fru_type_configs
      led_data[ "fruConfigs" ] = self.getFruLedConfigs()
      return json.dumps( led_data, indent=2 )
   ...
   def getFruLedConfigs( self ):
      fru_configs = []
      all_symlinks = self.parseSymbolicLinkToDevicePaths()
      inverse_symlink_paths = {v: k for k, v in all_symlinks.items()}

      for pm_unit in self.pmUnitConfigs:
         for slot_config in pm_unit.outgoingSlotConfigs:
            # We only care about FAN and PSU slots for this list
            if slot_config.slotType not in [ "FAN_SLOT", "PSU_SLOT" ]:
               continue
            
            fru_type = slot_config.slotType.replace( "_SLOT", "" )
            slot_index = int( slot_config.slotName.split( '@' )[ 1 ] )
            
            fru_name = f"{fru_type}{slot_index + 1}"
            
            presence_device_symlink = inverse_symlink_paths.get(slot_config.presenceDevicePath)

            if not presence_device_symlink:
               # Skip if we can't find the symlink for the presence device
               continue

            fru_entry = OrderedDict()
            fru_entry[ "fruName" ] = fru_name
            fru_entry[ "fruType" ] = fru_type
            fru_entry[ "presenceDetection" ] = {
               "sysfsFileHandle": {
                  "presenceFilePath": f"{presence_device_symlink}/{slot_config.presenceFileName}",
                  "desiredValue": 1,
               }
            }
            fru_configs.append(fru_entry)
