...
class PlatformConfig:
   def __init__( self, 
               platformName, 
               rootPmUnitName="SCM", 
               slotTypeConfigs=None,
               pmUnitConfigs=None, 
               i2cAdaptersFromCpu=[ { "adapter" : "SMBus I801 adapter at 1000" } ], 
               kmodsSettings={
...
class PmUnitConfig:
   ...
   def generateI2cAdapterSymlinks( self, platform ):
      ...
      if self.pmUnitName == "SCM":
         basePath = "/run/devmap/i2c-busses/MERU_SCM_CPLD"
         ...
         if platform == "meru800bia":
            ...
            basePath = "/run/devmap/i2c-busses/MERU800BFA_SMB_"
         pathPrefix = "/SMB_SLOT@0"
      if self.pciDeviceConfigs:
         for pciConfig in self.pciDeviceConfigs:
            if pciConfig.i2cAdapterConfigs:
               for i, adapterConfig in enumerate( pciConfig.i2cAdapterConfigs ):
                  for adapterNum in range( adapterConfig.numberOfAdapters ):
                     if platform == "meru800bia":
                        symlinkToDevicePaths[ f"{basePath}{i}_CH{adapterNum}" ] = \
                           f"{pathPrefix}/[{adapterConfig.pmUnitScopedName}@{adapterNum}]"
                     elif platform == "meru800bfa":
                        devName = adapterConfig.pmUnitScopedName.split('_', 2)[1] if self.pmUnitName == "SMB" else ""
                        symlinkToDevicePaths[ f"{basePath}{devName}_SMBUS{adapterConfig.pmUnitScopedName[-1]}_CH{adapterNum}" ] = \
                           f"{pathPrefix}/[{adapterConfig.pmUnitScopedName}@{adapterNum}]"
   ...
   def generateSensorSymlinks( self ):
      ...
      if self.pmUnitName == "SCM":
         ...
         if self.i2cDeviceConfigs:
            for config in self.i2cDeviceConfigs:
               name = config.pmUnitScopedName
               if "IDPROM" in name or "PCA" in name or "SMB_CPLD" in name:
                  continue
               symlinkToDevicePaths[ f"{ basePath }{ name }" ] = f"/SMB_SLOT@0/[{ name }]"
      return symlinkToDevicePaths
...
class PciDeviceConfig:
   ...
   def addI2cAdapterConfigs( self, numAdapters, adapterBaseName, baseCsrOffset ):
      ...
      for i in range( numAdapters ):
         if adapterStarCount == 2:
            ...
            adapterName = f"{adapterBaseName[:pos]}{deviceNum}{adapterBaseName[pos+1:rpos]}{i}{adapterBaseName[rpos+1:]}"
         elif adapterStarCount == 1:
            pos = adapterBaseName.find('*')
            adapterName = f"{adapterBaseName[:pos]}{i}{adapterBaseName[pos+1:]}"
         else:
            adapterName = adapterBaseName
         csrOffset = hex( int( baseCsrOffset, 16 ) + i * int( "0x80", 16 ) )
         configs.append( I2cAdapterConfig( adapterName, "i2c_master", \
                                          -1, csrOffset, 8 ) )
   ...
   def addXcvrCtrlConfigs( self, numConfigs, basePortNumber, portType="osfp", 
         xcvrBaseOffset="0xA010", led1BaseOffset="0x6100", led2BaseOffset="0x6110", 
         led3BaseOffset=None, led4BaseOffset=None, whistler=False ):
      if whistler:
         newConfigs = enumerateXcvrConfigsWhistler( numConfigs, basePortNumber, 
                     portType, xcvrBaseOffset, led1BaseOffset, led2BaseOffset, 
                     led3BaseOffset, led4BaseOffset )
      else:
         newConfigs = enumerateXcvrConfigsViper( numConfigs, basePortNumber, portType,
                     xcvrBaseOffset, led1BaseOffset, led2BaseOffset, led3BaseOffset, 
                     led4BaseOffset )
...
def enumerateXcvrConfigsViper( numConfigs, basePortNumber, portType, 
      xcvrBaseOffset, led1BaseOffset, led2BaseOffset, led3BaseOffset=None, 
      led4BaseOffset=None ):
   configs = []
...
class SCMUnit( PmUnitConfig ):
   def __init__( self ):
      ...
      self.addPciDeviceConfigs( [
         *enumeratePciDeviceConfigs( 1, "SCM_FPGA", "0x3475", "0x0001", "0x3475", "0x0008" )
      ] )

      self.pciDeviceConfigs[ 0 ].addI2cAdapterConfigs( 2, "SCM_I2C_MASTER*", "0x8000" )
