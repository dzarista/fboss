...
readonly INPUT_PORT_0=0x0
readonly OUTPUT_PORT_1=0x3
readonly CONFIG_REG_1=0x7
...
i2cset -f -y -m $BITMASK_PIN_1 $pca_bus $PCA_CHIP_ADDR $CONFIG_REG_1 0x0
# IO1_6 - SCD_RESET_L
i2cset -f -y -m $BITMASK_PIN_6 $pca_bus $PCA_CHIP_ADDR $CONFIG_REG_1 0x0
# IO1_0 - SCD_PRGM_L
i2cset -f -y -m $BITMASK_PIN_0 $pca_bus $PCA_CHIP_ADDR $CONFIG_REG_1 0x0

# Assert HITLESS_LATCH_L. This preserves the J3 reset state information and other information controlled by the SCD
i2cset -f -y -m $BITMASK_PIN_1 $pca_bus $PCA_CHIP_ADDR $OUTPUT_PORT_1 0x0
...
sleep 1
# Assert SCD_RESET_L
i2cset -f -y -m $BITMASK_PIN_6 $pca_bus $PCA_CHIP_ADDR $OUTPUT_PORT_1 0x0
...
i2cset -f -y -m $BITMASK_PIN_0 $pca_bus $PCA_CHIP_ADDR $OUTPUT_PORT_1 0x0
sleep 1
i2cset -f -y -m $BITMASK_PIN_0 $pca_bus $PCA_CHIP_ADDR $OUTPUT_PORT_1 0xFF

# Poll SCD_DONE(IO0_0). The pin will transition from 0 to 1 when the new image is loaded
programming_done_reg=$(i2cget -f -y $pca_bus $PCA_CHIP_ADDR $INPUT_PORT_0)
...
i2cset -f -y -m $BITMASK_PIN_6 $pca_bus $PCA_CHIP_ADDR $OUTPUT_PORT_1 0xFF
# De-assert HITLESS_LATCH_L
i2cset -f -y -m $BITMASK_PIN_1 $pca_bus $PCA_CHIP_ADDR $OUTPUT_PORT_1 0xFF
