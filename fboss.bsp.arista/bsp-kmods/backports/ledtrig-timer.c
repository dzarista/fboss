...
/*
 * Create the dummy module so platform_manager can also modprobe
 * "ledtrig_timer" in fbkernel 6.4 (when CONFIG_LEDS_TRIGGERS is
 * disabled).
 */
static int __init ledtrig_dummy_init(void) { return 0; }

static void __exit ledtrig_dummy_exit(void) {}

module_init(ledtrig_dummy_init);
module_exit(ledtrig_dummy_exit);

MODULE_AUTHOR("Tao Ren <taoren@meta.com>");
MODULE_DESCRIPTION("Dummy Timer LED trigger");
MODULE_LICENSE("GPL v2");
