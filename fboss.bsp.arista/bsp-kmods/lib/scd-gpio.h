...
#define SCD_GPIO_ATTR(_name, _mode, _show, _store, _ctx, _addr, _bit,          \
		      _active_low)                                             \
	{                                                                      \
		.dev_attr = __ATTR_NAME_PTR(_name, _mode, _show, _store),      \
		.ctx = _ctx, .addr = _addr, .bit = _bit,                       \
		.active_low = _active_low                                      \
	}

#define SCD_RW_GPIO_ATTR(_name, _ctx, _addr, _bit, _active_low)                \
	SCD_GPIO_ATTR(_name, S_IRUGO | S_IWUSR, attribute_gpio_get,            \
		      attribute_gpio_set, _ctx, _addr, _bit, _active_low)

#define SCD_RO_GPIO_ATTR(_name, _ctx, _addr, _bit, _active_low)                \
	SCD_GPIO_ATTR(_name, S_IRUGO, attribute_gpio_get, NULL, _ctx, _addr,   \
		      _bit, _active_low)
